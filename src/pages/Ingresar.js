...
import { getInventory, setInventory, addToHistory, generateMovementPDF, getConfig, getPendingMovements, setPendingMovements, clearPendingMovements } from "../utils/localStorage";  
...
const Ingresar = () => {  
    ...
    useEffect(() => {  
        ...
        const pending = getPendingMovements('entradas');  
        setMovements(pending);  
        setObservations(pending.observations || ""); // Asumir que observaciones se guardan por separado si needed  
    }, []);  
    ...
    const addToMovements = (e) => {  
        ...
        setPendingMovements(updatedMovements, 'entradas');  
        setSelectedItem("");  
        setSelectedTalla("");  
        setCantidad(1);  
    };  
    ...
    const handleSubmit = async (e) => {  
        ...
        if (generatePDF) {  
            generateMovementPDF(movements, config, "entrada", observations);  
        }  

        // Limpiar pendientes  
        clearPendingMovements('entradas');  
        setMovements([]);  
        setObservations("");  
        setGeneratePDF(false);  
        alert("¡Ingreso procesado! PDF generado si lo seleccionaste.");  
    };  

    return (  
        <motion.div className="p-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>  
            <h2 className="text-3xl font-bold text-gray-800 mb-6">Ingresar Items</h2>  
            ...
            {movements.length > 0 && (  
                <div className="bg-white rounded-2xl p-6 shadow-lg mb-6">  
                    <h3 className="text-lg font-semibold mb-4">Lista de Ingresos ({movements.length})</h3>  
                    <div className="space-y-2 mb-4">  
                        {movements.map((mov, index) => (  
                            <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-xl">  
                                <span>{mov.item} - {mov.talla}: {mov.cantidad}</span>  
                                <motion.button  
                    ...
                    <motion.button  
                        ...
                        className="flex items-center gap-2 px-6 py-3 bg-green-500 text-white rounded-xl font-semibold"  
                        whileHover={{ scale: 1.05 }}  
                    >  
